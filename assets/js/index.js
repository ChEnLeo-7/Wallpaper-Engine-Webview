...
function setPendingDownloadAndReload(api, fid, title){
  try{
    sessionStorage.setItem('wallhub_autodl', JSON.stringify({ fid:String(fid||''), title:String(title||''), at:Date.now() }));
  }catch{}
  const u = new URL(location.href);
  u.searchParams.set('api', String(api||'').trim().replace(/\/+$/,''));
...
function dlWall(fid, title){
  if(!inBackendMode()){
    const cur = (localStorage.getItem('wallhub_api_base') || '').trim();
    const hint = cur ? `\n\n当前已保存：${cur}` : '';
    const api = prompt(
      '下载需要后端服务（wallhub2/server.js）。\n请输入后端地址，例如：\n- https://你的后端域名\n- http://localhost:3080\n\n提示：GitHub Pages 是 https，直连本机 http 可能会被浏览器拦截；推荐把后端部署成 https，或用隧道转成 https。' + hint,
      cur || ''
    );
    const v = (api || '').trim().replace(/\/+$/,'');
    if(!v) return;
    localStorage.setItem('wallhub_api_base', v);
    setPendingDownloadAndReload(v, fid, title);
    return;
  }
