...
let DETAIL_OBSERVER = null;
const DETAIL_QUEUE = [];
const DETAIL_SEEN = new Set();
let DETAIL_RUNNING = false;
...
/* ================================================================
   CORE FUNCTIONS (INIT & LOAD)
================================================================ */
function initApp(isPopup = false) {
  S.isPopup = isPopup;
  renderGenreGrid();
  setupEvents();
  syncDaysVisible();
  ...
  try{
    const autodl = sessionStorage.getItem('wallhub_autodl');
    if(autodl){
      sessionStorage.removeItem('wallhub_autodl');
      const j = JSON.parse(autodl);
      if(j && j.fid && inBackendMode()){
        setTimeout(()=>dlWall(j.fid, j.title||''), 600);
      }
    }
  }catch{}
...
async function load(){
  ...
  try {
    ...
    if(!list.length){
      ...
      // Detailed enrichment (skip for popup if desired, but kept for consistency)
      if(!inBackendMode()){
        setupLazyDetailObserver();
        enrichItemsViaProxy(S.items, loadToken).catch(()=>{});
      }
    }
  } catch(err){
    console.error(err);
...
function renderItems(items){
  ...
  if(!inBackendMode()) setupLazyDetailObserver();
}
...
function cardHtml(item, isL, idx){
  ...
  const subs  = fmtN(item.subscriptions ?? item.lifetime_subscriptions);
  const views = fmtN(item.views);
  const typeCls = type==='未知' ? 'unknown' : type.toLowerCase();
  const delay = Math.min(idx*25,400);

  return `
  <div class="card ${isL?'lv':''}" style="animation-delay:${delay}ms" onclick="openModal('${fid}')" data-fid="${fid}">
    <div class="card-thumb">
      <div class="skel"></div>
      <img data-src="${thumb||'PLACEHOLDER'}" data-id="${fid}" alt="${esc(title)}" loading="lazy">
      <span class="type-badge ${typeCls}" id="type-${fid}">${type}</span>
    </div>
    <div class="card-body">
      ...
      <div class="card-meta">
        <span class="cstat">
          ...
          <span id="subs-${fid}">${subs}</span>
        </span>
        <span class="cstat">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
          <span id="views-${fid}">${views}</span>
        </span>
      </div>
    </div>
    <div class="card-foot">
...
function openModal(id){
  ...
  if(!inBackendMode()){
    fetchProxyDetail(id).then(d=>{
      if(S.modalId !== id) return;
      ...
      if(d.subscriptions !== null && d.subscriptions !== undefined) item.subscriptions = d.subscriptions;
      if(d.favorited !== null && d.favorited !== undefined) item.favorited = d.favorited;
      if(d.views !== null && d.views !== undefined) item.views = d.views;
      if(d.file_size) item.file_size = d.file_size;
      if(d.time_updated) item.time_updated = d.time_updated;
      if(d.preview_url) item.preview_url = d.preview_url;
      if(d.tags && d.tags.length) item.tags = d.tags.map(t=>({tag:t}));
      updateCard(id, item);
      renderStats({
        subs:  fmtN(d.subscriptions ?? item.subscriptions),
        favs:  fmtN(d.favorited ?? item.favorited),
        views: fmtN(d.views ?? item.views),
        size:  d.file_size || '未知',
...
function renderCmts(list){
  ...
  el.innerHTML=list.map(c=>`
    <div class="cmt">
      <div class="cmt-head">
        <span class="cmt-author">${esc(stripHtml(normalizeUserText(c.author||'Steam用户')))}</span>
        <span class="cmt-date">${esc(normalizeUserText(c.date||''))}</span>
      </div>
      <div class="cmt-text">${formatCommentText(normalizeUserText(c.text||''))}</div>
    </div>`).join('');
}
...
function formatCommentText(s) {
  let v = esc(s);
  v = v.replace(/\n/g, '<br>');
  v = v.replace(/(https?:\/\/[^\s<]+)/g, '<a href="$1" target="_blank" rel="noopener noreferrer" style="color:var(--accent);text-decoration:none">$1</a>');
  return v;
}
...
function getType(item){
  ...
  if(rawType){
    if(rawType.includes('video') || rawType.includes('视频')) return 'Video';
    if(rawType.includes('scene') || rawType.includes('场景')) return 'Scene';
    if(rawType.includes('application') || rawType.includes('程序') || rawType.includes('应用')) return 'Application';
    if(rawType.includes('web') || rawType.includes('网页') || rawType.includes('网站')) return 'Web';
  }
  const ts=(item.tags||[]).map(t=>String(t.tag||t||'').toLowerCase()).filter(Boolean);
  if(!ts.length) return '未知';
  if(ts.some(t=>t.includes('video')||t.includes('视频'))) return 'Video';
  if(ts.some(t=>t.includes('scene')||t.includes('场景'))) return 'Scene';
  if(ts.some(t=>t.includes('application')||t.includes('程序')||t.includes('应用'))) return 'Application';
  if(ts.some(t=>t.includes('web')||t.includes('网页')||t.includes('网站'))) return 'Web';
  return '未知';
}
...
function dlWall(fid, title){
  if(!inBackendMode()){
    const choice = confirm('检测到未配置后端服务。\n\n点击“确定”前往第三方下载页获取文件（推荐）。\n点击“取消”前往 Steam 官方订阅页面。');
    if(choice){
       window.open(`http://steamworkshop.download/download/view/${fid}`, '_blank');
       toast('已跳转至下载页', 'ok');
    } else {
       const page = `https://steamcommunity.com/sharedfiles/filedetails/?id=${fid}`;
       window.open(page, '_blank');
       toast('已跳转至 Steam 订阅页', 'ok');
    }
    return;
  }
...
function proxyTipHtml(msg){
  ...
  const rateLimit = /\b429\b/.test(m);
  const title = badApi ? '🧩 当前是后端接口配置问题' : rateLimit ? '⛔ 公开代理被限流（HTTP 429）' : '🌐 当前网络可能受限，请开启代理后再访问';
  const desc = badApi ? '检测到后端接口不可用。请确认你配置的 ?api 地址支持 POST /api/steam/query。' : rateLimit ? '当前默认公开代理无法使用。请切换到你自己的代理或配置后端接口。' : '检测到请求 Steam 社区服务失败。请先开启 VPN/代理，再点击重试。';
  ...
  return `
    <div class="proxy-tip">
      ...
      <div class="proxy-desc" style="margin-top:10px">
        <div style="font-weight:700;color:var(--text);margin-bottom:6px">后端 API（推荐，支持下载）</div>
        <input id="apiInp" value="${esc(API_BASE||'')}" placeholder="例如：https://你的后端域名" style="width:100%;background:var(--bg3);border:1px solid var(--border);border-radius:8px;padding:9px 10px;color:var(--text);font-family:inherit;font-size:12px;outline:none">
        <div style="height:8px"></div>
        <div style="font-weight:700;color:var(--text);margin-bottom:6px">公开代理模板（可选）</div>
        <input id="proxyInp" value="${esc(PROXY_TEMPLATE||'')}" placeholder="例如：https://r.jina.ai/http:// 或 https://xxx/?url={url}" style="width:100%;background:var(--bg3);border:1px solid var(--border);border-radius:8px;padding:9px 10px;color:var(--text);font-family:inherit;font-size:12px;outline:none">
      </div>
      <div class="proxy-actions">
        <button class="proxy-btn" onclick="saveApiProxyAndReload()">保存并重试</button>
        <button class="proxy-btn alt" onclick="copyProxyDomains()">复制代理域名</button>
      </div>
    </div>`;
}
function saveApiProxyAndReload(){
  const api = (document.getElementById('apiInp')?.value || '').trim().replace(/\/+$/,'');
  const proxy = (document.getElementById('proxyInp')?.value || '').trim();
  if(api) localStorage.setItem('wallhub_api_base', api);
  else localStorage.removeItem('wallhub_api_base');
  if(proxy) localStorage.setItem('wallhub_proxy_template', proxy);
  else localStorage.removeItem('wallhub_proxy_template');
  location.reload();
}
...
/* ================================================================
   ENRICHMENT (DETAIL PARSING)
================================================================ */
function parseSteamNumber(v){
  const s = String(v||'').replace(/[^\d]/g,'');
  if(!s) return null;
  const n = parseInt(s, 10);
  return Number.isFinite(n) ? n : null;
}
function parseSteamDetailMarkdown(md, id){
  const preview = (md.match(/https:\/\/images\.steamusercontent\.com\/ugc\/[^)\s]+/i)||[])[0] || '';
  const type = (md.match(/Type:\[([^\]]+)\]/i)||[])[1]
    || (md.match(/类型[:：]\s*\[?([^\]\n]+)\]?/i)||[])[1]
    || '';
  const rating = (md.match(/Age Rating:\[([^\]]+)\]/i)||[])[1]
    || (md.match(/年龄分级[:：]\s*\[?([^\]\n]+)\]?/i)||[])[1]
    || '';
  const genre = (md.match(/Genre:\[([^\]]+)\]/i)||[])[1]
    || (md.match(/风格[:：]\s*\[?([^\]\n]+)\]?/i)||[])[1]
    || '';
  const views = parseSteamNumber((md.match(/([\d,]+)\s+(?:Unique Visitors|独立访客|独立访问者|访客)/i)||[])[1]);
  const subs = parseSteamNumber((md.match(/([\d,]+)\s+(?:Current Subscribers|当前订阅|订阅者|订阅)/i)||[])[1]);
  const favs = parseSteamNumber((md.match(/([\d,]+)\s+(?:Current Favorites|当前收藏|收藏)/i)||[])[1]);
  const size = ((md.match(/(?:File Size|文件大小)[\s\S]{0,240}?(\d[\d.,]*\s*(?:TB|GB|MB|KB|B))/i)||[])[1] || '').trim();
  const updated = ((md.match(/(?:Last Updated|最后更新|更新时间|更新日期)[:：]?\s*([^\n]+)/i)||[])[1] || '').trim()
    || ((md.match(/(?:File Size|文件大小)[\s\S]{0,360}?\n\s*\d[\d.,]*\s*(?:TB|GB|MB|KB|B)\s*\n+\s*([^\n]+?@[^\n]+|[A-Za-z]{3}\s+\d{1,2}[^\n]*|[0-9]{4}[^\n]*)/i)||[])[1] || '').trim();
  const author = decodeHtmlEntities(((md.match(/(?:Created by|创建者|作者)[\s\S]{0,340}\n\s*([^\n]+)\n\s*(?:Offline|Online|离线|在线)\b/i)||[])[1] || '').trim());
  const desc = decodeHtmlEntities(((md.match(/(?:Description|描述)\s*\n+([\s\S]*?)(?:\n+\d+\s+(?:Comments|评论)|\n+\[\<\]|\n+Edit links|\n+编辑链接)/i)||[])[1] || '').trim());
  const comments = [];
  const cRe = /\[([^\]]+)\]\(https?:\/\/steamcommunity\.com\/(?:profiles|id)\/[^)]+\)\s+([^\n]+)\n\n([\s\S]*?)(?=\n\s*\[![^\n]+\]\(https?:\/\/avatars\.|\n+\[\<\]|\n+$)/g;
  let m;
  while((m = cRe.exec(md))){
    const text = decodeHtmlEntities(m[3].trim());
    if(!text) continue;
    comments.push({ author:decodeHtmlEntities(m[1].trim()), date:decodeHtmlEntities(m[2].trim()), text });
    if(comments.length >= 30) break;
  }
  const tags = [type, rating, genre].filter(Boolean);
  return {
    publishedfileid:id,
    preview_url: preview,
    tags,
...
async function fetchProxyDetail(id){
  if(DETAIL_CACHE.has(id)) return DETAIL_CACHE.get(id);
  const lk = `wallhub_detail_${id}`;
  try{
    const cached = localStorage.getItem(lk);
    if(cached){
      const obj = JSON.parse(cached);
      if(obj && obj._t && (Date.now() - obj._t) < 1000*60*60*24){
        DETAIL_CACHE.set(id, obj);
        return obj;
      }
    }
  }catch{}
  const page = `https://steamcommunity.com/sharedfiles/filedetails/?id=${id}`;
  const res = await fetch(proxyUrl(page), { cache:'no-store' });
  if(!res.ok) throw new Error(`detail ${res.status}`);
  const md = await readTextUtf8(res);
  const d = parseSteamDetailMarkdown(md, id);
  DETAIL_CACHE.set(id, d);
  try{ localStorage.setItem(lk, JSON.stringify(Object.assign({_t:Date.now()}, d))); }catch{}
  return d;
}

function setupLazyDetailObserver(){
  if(inBackendMode()) return;
  if(DETAIL_OBSERVER) DETAIL_OBSERVER.disconnect();
  DETAIL_OBSERVER = new IntersectionObserver(es=>{
    es.forEach(e=>{
      if(!e.isIntersecting) return;
      const fid = e.target?.dataset?.fid;
      if(!fid || DETAIL_SEEN.has(fid)) return;
      DETAIL_QUEUE.push(fid);
      drainDetailQueue().catch(()=>{});
    });
  }, {rootMargin:'280px'});
  document.querySelectorAll('.card[data-fid]').forEach(el=>DETAIL_OBSERVER.observe(el));
}
async function drainDetailQueue(){
  if(DETAIL_RUNNING) return;
  DETAIL_RUNNING = true;
  try{
    const max = 4;
    ...
    const runOne = async (id) => {
      try{
        const d = await fetchProxyDetail(id);
        const it = S.items.find(x=>String(x.publishedfileid)===String(id));
        if(it){
          if(d.subscriptions !== null && d.subscriptions !== undefined) it.subscriptions = d.subscriptions;
          if(d.favorited !== null && d.favorited !== undefined) it.favorited = d.favorited;
          if(d.views !== null && d.views !== undefined) it.views = d.views;
          if(d.file_size) it.file_size = d.file_size;
          if(d.time_updated) it.time_updated = d.time_updated;
          if(d.preview_url) it.preview_url = d.preview_url;
          if(d.type) it.type = d.type;
          if(d.tags && d.tags.length) it.tags = d.tags.map(t=>({tag:t}));
          updateCard(id, it);
          if(S.modalId === id){
            renderStats({
              subs:  fmtN(it.subscriptions),
              favs:  fmtN(it.favorited),
              views: fmtN(it.views),
              size:  it.file_size || '未知',
              upd:   it.time_updated || '未知',
              id,
            });
          }
        }
        ...
        if(/\b429\b/.test(msg)){
          DETAIL_SEEN.delete(id);
          DETAIL_QUEUE.length = 0;
        }
      }
      await sleep(120);
    };
    while(DETAIL_QUEUE.length || running.size){
      while(DETAIL_QUEUE.length && running.size < max){
        const id = DETAIL_QUEUE.shift();
        if(!id || DETAIL_SEEN.has(id)) continue;
        DETAIL_SEEN.add(id);
        const p = runOne(id).finally(()=>running.delete(p));
        running.add(p);
      }
...
async function enrichItemsViaProxy(items, token){
  const queue = (items||[]).map(it=>String(it.publishedfileid||'')).filter(Boolean);
  const conc = 2;
  let idx = 0;
  let stop = false;
  async function worker(){
    while(idx < queue.length){
      if(S.loadToken !== token) { stop = true; return; }
      if(stop) return;
      const id = queue[idx++];
      try{
        const d = await fetchProxyDetail(id);
        const it = S.items.find(x=>String(x.publishedfileid)===id);
        if(!it) continue;
        if(d.subscriptions !== null && d.subscriptions !== undefined) it.subscriptions = d.subscriptions;
        if(d.favorited !== null && d.favorited !== undefined) it.favorited = d.favorited;
        if(d.views !== null && d.views !== undefined) it.views = d.views;
...
function updateCard(fid, item){
  const subsEl = document.getElementById(`subs-${fid}`);
  if(subsEl) subsEl.textContent = fmtN(item.subscriptions ?? item.lifetime_subscriptions);
  const viewsEl = document.getElementById(`views-${fid}`);
  if(viewsEl) viewsEl.textContent = fmtN(item.views);
  const typeEl = document.getElementById(`type-${fid}`);
  if(typeEl){
    const t = getType(item);
    typeEl.textContent = t;
    typeEl.className = `type-badge ${t==='未知'?'unknown':t.toLowerCase()}`;
  }
}
